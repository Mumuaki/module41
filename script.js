...
function scheduleReconnect() {
  ...
  reconnectTimeout = setTimeout(() => {
    if (currentEndpointIndex < WS_ENDPOINTS.length - 1 && !endpointFallbackUsed) {
      currentEndpointIndex += 1;
      endpointFallbackUsed = true;
      addMessage(
        "Основной сервер недоступен. Переключаемся на резервный канал.",
        "server"
      );
    }
    connectSocket();
...
function handleConnectionIssue(message, statusText) {
  if (!connectionIssueShown) {
    addMessage(message, "server");
    connectionIssueShown = true;
  }
  setChatStatus("error", statusText);
  scheduleReconnect();
}
...
function connectSocket() {
  ...
  socket = new WebSocket(WS_ENDPOINTS[currentEndpointIndex]);
  ...
  socket.addEventListener("error", () => {
    handleConnectionIssue("Ошибка соединения с сервером", "Ошибка соединения. Переподключаемся...");
  });
  ...
  socket.addEventListener("close", () => {
    ...
    handleConnectionIssue("Соединение закрыто", "Соединение закрыто. Переподключаемся...");
  });
}
